...
Use Secret Manager secret *names* rather than secret values. Full deployments require target-specific names for the SSH key and VNC password; automated-login targets also require Gateway username and password names. TOTP auto-fill is off by default; an explicit opt-in also requires a target-specific TOTP secret name. When auto-fill is off, the workflow does not fetch or send the TOTP seed to the VM. Do not commit target JSON, credentials, account identifiers, addresses, or private keys. A missing target configuration fails before any cloud authentication or remote operation.

Full deployments install the VM runtime `.env` with mode `0600`. Set `manual_auth: true`, `ibkr_2fa_autofill: "no"`, and `maintenance_enabled: false` for human authentication. A full deploy then installs only the VNC credential, starts the container, leaves recovery timers disabled, and reports `GATEWAY_MANUAL_AUTH_REQUIRED=true`; it does not establish broker or API readiness. Complete broker login through a temporary IAP/SSH VNC tunnel, then verify API and account identity before enabling platform traffic. The default automated-login path still stores a Gateway password on the VM; it is unsuitable where fully manual authentication is required.
