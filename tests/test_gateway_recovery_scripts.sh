...
grep -Fq 'run_stage_assessment()' "$recover_script"
grep -Fq 'epoch_matches_current()' "$recover_script"
grep -Fq 'snapshot_not_before()' "$recover_script"
...
grep -Fq 'Container identity/StartedAt changed; discarding snapshot for a new epoch assessment.' "$recover_script"
grep -Fq 'Each evaluation freezes one bounded snapshot.' "$recover_script"
grep -Fq 'event_not_before="$(snapshot_not_before' "$recover_script"
grep -Fq "docker logs --timestamps --since \"\${event_not_before}\" \"\${epoch_container_id}\"" "$recover_script"
grep -Fq "docker exec \"\${epoch_container_id}\"" "$recover_script"
grep -Fq "[ -e \"\${log_path}\" ] && [ ! -f \"\${log_path}\" ]" "$recover_script"
grep -Fq 'tail -n 400 "${log_path}"' "$recover_script"
...
grep -Fq 'bash tests/test_gateway_recovery_runtime_contract.sh' "$ci_workflow"
