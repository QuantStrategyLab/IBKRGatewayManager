...
grep -Fq 'ARG IB_GATEWAY_BASE_VERSION=10.50.1e' "$dockerfile"
grep -Fq 'FROM gnzsnz/ib-gateway:${IB_GATEWAY_BASE_VERSION}' "$dockerfile"
...
grep -Fq 'IB_GATEWAY_BASE_VERSION: ${IB_GATEWAY_BASE_VERSION:-10.50.1e}' "$compose_file"
...
grep -Fq '      - IBKR_2FA_AUTOFILL=${IBKR_2FA_AUTOFILL:-no}' "$compose_file"
