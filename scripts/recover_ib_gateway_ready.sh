...
gateway_recent_activity_from_file_logs() {
...
gateway_recent_activity() {
  {
    gateway_recent_activity_from_docker_logs || true
    gateway_recent_activity_from_file_logs || true
...
wait_for_ready_with_progress() {
  ...
  while [ "${extension}" -lt "${progress_extensions}" ]; do
    activity="$(gateway_recent_activity)"
    case "${activity}" in
      terminal)
